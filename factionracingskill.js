...
(function () {
  ...
  // ── Popup Modal ────────────────────────────────────────────────────────────
  function openPopup() {
    // Backdrop overlay layer that explicitly isolates the popup card
    ...
    // Shrunk Floating Pop-up Window Box Configuration
    ...
    // Tap backdrop OR close button to remove it
  ...
  async function fetchData() {
    ...
    if (!factionData || factionData.error) {
      const body = document.getElementById('nb-body');
      if (body) body.innerHTML = `<p style="color:red; padding:1rem;">API error: ${factionData?.error?.error ?? 'Unknown'}</p>`;
      return;
    }
